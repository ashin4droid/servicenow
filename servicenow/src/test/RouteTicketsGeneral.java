...
import com.wipro.automation.snow.pages.HomePageTest;
...
import com.wipro.automation.snow.utils.QueueDataNew;
...
public class RouteTicketsGeneral
{
	...
	public static List<QueueDataNew> qDataNew = null;
	
	
	
	public static void route() throws Exception
	{
		Thread dataLoadThread = new Thread(new LoadData());
		...
		lib.SwitchToFrame("gsft_main");
		List<WebElement> incList = driver.findElements(By.xpath("//tbody[@class='list2_body']/tr/td[3]/a"));
		int ticketCount = incList.size();
		
		List<String> ticketsToBeRouted = new ArrayList<>();
		for (WebElement webElement : incList)
		{
			ticketsToBeRouted.add(webElement.getText().trim());
		}
		int itrCount = ticketsToBeRouted.size();
		...
		inc : for (int i = 1; i <= itrCount; i++)
		{
			...
			if (lib.ClickOnWebElement(By.xpath("//tbody[@class='list2_body']/tr/td[3]/a[contains(text(),'"+ ticketsToBeRouted.get(i-1) +"')]"))){}
			else {
				continue;
			}
			...
			for (QueueDataNew queueData : qDataNew)
			{
				if (shortDesc.contains(queueData.getKeyword().trim()))
				{
					...
					JSWaiter.waitUntilAngularReady();
					lib.SendTextToWebElement(GLSAPPage.assignedTo, queueData.getAssignee());
					lib.ClickOnWebElement(By.xpath("//div[@id='AC.incident.assigned_to']/table/tbody/tr/td[text()='"+ queueData.getAssignee() +"']"));
